...
from .views import (
    ...
    ProductDeleteView,
    ProductUpdateView,
    ExtremeWidgetCreateView,
    PropertyOwnerCreateView,
    PropertyCreateView,
    PropertyUpdateView,
    EmployeeCreateView,
    EmployeeUpdateView,
...
def __setup_admins():
    ...
    admin = User(
        username="admin@example.com", is_staff=True, is_superuser=True
    )
    admin.set_password("admin")
    admin.save()
    admin = User(
        username="admin1@example.com", is_staff=True, is_superuser=True
    )
...
class LogEntryMetaOptionsTest(TestCase):
    def test_app_label(self):
        self.failUnlessEqual(
            Product.audit_log.model._meta.app_label, Product._meta.app_label
        )
        self.failUnlessEqual(
            WarehouseEntry.audit_log.model._meta.app_label,
            WarehouseEntry._meta.app_label,
        )

    def test_table_name(self):
        self.failUnlessEqual(
            Product.audit_log.model._meta.db_table,
            "%sauditlogentry" % Product._meta.db_table,
        )
        self.failUnlessEqual(
            WarehouseEntry.audit_log.model._meta.db_table,
...
class TrackingAuthFieldsTest(TestCase):
    ...
    def setUp(self):
        category = ProductCategory.objects.create(
            name="gadgets", description="gadgetry"
        )
        category.product_set.create(
            name="new gadget", description="best gadget eva", price=100
        )
    ...
    def test_logging_user(self):
        ...
        self.assertEqual(
            product.productrating_set.all()[0].user.username,
            "admin@example.com",
        )
...
class TrackingChangesTest(TestCase):
    ...
    def test_logging_insert_anon(self):
        ...
        self.assertEqual(
            category.audit_log.all()[0].description, category.description
        )
    ...
    def test_logging_insert_auth(self):
        ...
        self.assertEqual(
            category.audit_log.all()[0].action_user.username,
            "admin@example.com",
        )
    ...
    def test_loggin_update_anon(self):
        ...
        self.assertGreater(
            product.audit_log.all()[0].action_date,
            product.audit_log.all()[1].action_date,
        )
        self.assertEqual(product.audit_log.all()[1].action_type, "I")
        self.assertEqual(product.audit_log.all()[0].action_type, "U")
        self.assertEqual(
            product.audit_log.all()[0].description, "Test description new"
        )
    ...
    def test_loging_update_auth(self):
        ...
        self.assertEqual(
            product.audit_log.all()[0].action_user.username,
            "admin@example.com",
        )
    ...
    def test_logging_inherited(self):
        ...
        c.login(username="admin@example.com", password="admin")
        c.post(
            "/extremewidget/create/",
            {"name": "Test name", "special_power": "Testpower"},
        )
        widget = ExtremeWidget.objects.all()[0]
        self.failUnlessEqual(widget.audit_log.all()[0].name, "Test name")
        self.failUnlessEqual(
            hasattr(widget.audit_log.all()[0], "special_power"), True
        )
        self.failUnlessEqual(
            widget.audit_log.all()[0].special_power, "Testpower"
        )
    ...
    def test_logging_custom_user(self):
        ...
        c.post(
            "/employee/create/",
            {"email": "vvangelovski@gmail.com", "password": "testpass"},
        )
...
class TestOneToOne(TestCase):
    ...
    def run_client(self, client):
        ...
        client.post(
            "/property/create/", {"name": "Property1", "owned_by": "1"}
        )
        client.post(
            "/property/update/1/", {"name": "Property2", "owned_by": "2"}
        )
    ...
    def test_fields(self):
        ...
        self.assertEqual(
            prop.audit_log.all()[0]._meta.get_field("owned_by").__class__,
            models.ForeignKey,
        )
