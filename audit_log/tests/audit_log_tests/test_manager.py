...
class DisablingTrackingTest(TestCase):
    def test_disable_enable_instance(self):
        ...
        ProductCategory.objects.create(
            name="test category2", description="test"
        )
    ...
    def test_disable_enable_class(self):
        self.assertRaises(
            ValueError, ProductCategory.audit_log.disable_tracking
        )
        self.assertRaises(
            ValueError, ProductCategory.audit_log.enable_tracking
        )
        self.assertRaises(
            ValueError, ProductCategory.audit_log.is_tracking_enabled
        )
